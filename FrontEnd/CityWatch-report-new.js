...
if (form) {
  ...
  if ((!title || title.length === 0) && category && category !== 'other'){
      ...
      // Prefer fullAddress when provided (the textarea), otherwise require a selected location.
      if (!fullAddress && !location) {
        showStatus('Please provide a full address or select a location for the report.', 'error');
        setLoading(false);
        return;
      }

      // If user selected 'Other', ensure they provided the text in the location-other input
      if (location === 'Other') {
  ...
  // Handle category - send custom category if 'other' is selected
  if (category === 'other' && document.getElementById('custom-title') && document.getElementById('custom-title').value.trim()) {
    formData.append('category', document.getElementById('custom-title').value.trim());
  } else if (category) {
  ...
  formData.append('priority', 'medium'); // Default priority
      ...
      // Send address: prefer the fullAddress textarea if provided, otherwise the selected location (or location-other)
      if (fullAddress) {
        formData.append('address', fullAddress);
      } else if (location === 'Other' && locationOtherEl && locationOtherEl.value.trim()) {
        formData.append('address', locationOtherEl.value.trim());
      } else if (location) {
        formData.append('address', location);
      }

      // Also send the selected barangay (location) separately so admin can display both
      if (location === 'Other' && locationOtherEl && locationOtherEl.value.trim()) {
        formData.append('barangay', locationOtherEl.value.trim());
      } else if (location) {
        formData.append('barangay', location);
      }
